        ...
        <v-dialog v-model="dialog" max-width="400" content-class="shadow-sm" overlay-opacity=".25" eager scrollable persistent>
            <v-form @submit.prevent="submit" :disabled="loading">
                <v-card>
                    <v-toolbar flat>
                        <v-subheader>
                            Konfirmasi Hapus data Kategori > {{ ori.nama_kategori }}
                        </v-subheader>
                        <v-spacer/>
...
export default {
    ...
    computed: {
        ...mapState({
            value_dialog: state => state.kategori.modal.hapus,
    ...
    methods: {
        ...mapActions({
            setDialog: 'kategori/setModalHapus',
            deleteKategori: 'kategori/destroy',
        ...
        async submit(e){
            ...
            this.loading = true
            let res = await this.deleteKategori({ data, id: this.id }).catch(e => {
                console.log("deleteKategori@KategoriTambah.vue", e);
        ...
        async loadItem(){
            ...
            let res = await this.findKategori({ id: this.id }).catch(e => {
                console.log("loadItem@KategoriTambah.vue", e);
                this.notif({
                    message: e.message
                })
            })
