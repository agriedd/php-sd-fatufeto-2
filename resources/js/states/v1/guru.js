    ...
    actions: {
        get(context, params = {}){
            return new Promise(async(resolve, reject)=>{
                let res = await axios.get(api('v1/guru'), { params: params }).catch(e => reject(e))
                if(res) resolve(res)
            })
        },
        count(context, params = {}){
            return new Promise(async(resolve, reject)=>{
                let res = await axios.get(api('v1/analytic/guru/count'), { params: params }).catch(e => reject(e))
                if(res) resolve(res)
            })
        },
        async show(context, {data = {}, id}){
            if(id)
                return new Promise(async(resolve, reject)=>{
                    let res = await axios.get(api(`v1/guru/${id}`), { params: data }).catch(e => reject(e))
                    if(res) resolve(res)
                })
            else
                console.warn("show@guru.js", "id kosong 🤦‍♂️");
        },
        store(context, data){
            return new Promise(async(resolve, reject)=>{
                let res = await axios.post(api('v1/guru'), data).catch(e => reject(e))
                if(res) resolve(res)
            })
        },
        async update(context, { data, id }){
            if(id){
                ...
                return new Promise(async(resolve, reject)=>{
                    let res = await axios.post(api(`v1/guru/${id}`), data).catch(e => reject(e))
                    if(res) resolve(res)
                })
            }
            else
        ...
        async destroy(context, { data, id }){
            if(id){
                ...
                console.warn("destroy@guru.js", "id kosong 🤦‍♂️");
        },
                ...
                async reset(context, { data, id }){
                    if(id){
                        if(data instanceof FormData)
                            data.append('_method', 'PUT')
                        return new Promise(async(resolve, reject)=>{
                            let res = await axios.post(api(`v1/guru/reset/${id}`), data).catch(e => reject(e))
                            if(res) resolve(res)
                        })
                    }
                    else
