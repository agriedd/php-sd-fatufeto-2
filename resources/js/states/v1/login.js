...
export default {
	...
	getters: {
		isAdmin: (state, getters, root) => {
			return getters['admin/exists'];
		},
		isPimpinan: (state, getters, root) => {
	...
	modules: {
		...
		guru: {
			...
			actions: {
				check(context, { token = null }){
					return new Promise(async(resolve, reject)=>{
						let res = await axios.post(api('v1/guru/user/authorize'), {}, { headers: { Authorization: `Bearer ${token}` } }).catch(e => reject(e))
						if(res) resolve(res)
					})
				},
				logout(context, data){
		...
		pimpinan: {
			...
			actions: {
				check(context, { token = null }){
					return new Promise(async(resolve, reject)=>{
						let res = await axios.post(api('v1/pimpinan/user/authorize'), {}, { headers: { Authorization: `Bearer ${token}` } }).catch(e => reject(e))
						if(res) resolve(res)
					})
				},
				logout(context, data){
